...
def generate_features(clean_data: pd.DataFrame, feature_config: dict) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info("Dropped columns: %s", feature_config.get('drop_columns', []))
    ...
    try:
        ...
        logger.info("Filtered airlines with less than %s flights", feature_config.get('filter_airlines', 1000))
    ...
    try:
        ...
        logger.info("Log transformed columns: %s", feature_config.get('log_transform', []))
