...
def get_duration(text: str) -> float:
    """
    Extracts the duration of an event in hours from a given string. The function assumes that the input string contains
    information about the duration in a format like "5.30h m" or "5h 30m".
...
def get_stops(text: str, pattern: str, stop_dict: dict) -> int:
    """
    Extracts the number of stops from a given string using the provided regular expression pattern. The function then 
    uses a provided dictionary to map the extracted string to an integer.

    Args:
        text (str): The string containing the stops information.
        pattern (str): The regular expression pattern to extract the stops information from the text.
...
def clean_data(sso_profile: str, bucket_name: str, raw_key: str, clean_config: dict) -> pd.DataFrame:
    ...
    Args:
        ...
        clean_config (dict): Configuration for cleaning and transforming the data. Should have the following keys:
            ...
            - "get_stops": A dictionary specifying the pattern to match in the stops column and a dictionary to map the
                           current stop categories to new categories.
            - "price_cols": A dictionary specifying columns with price information and their corresponding pattern to 
                           remove and its replacement before converting to integer.
    ...
    for new_col, bucket_col in clean_config["bucket_time_cols"].items():
        df_clean[new_col] = raw_data[bucket_col].apply(lambda x: bucket_hours(x, clean_config["bucket_hours"]))
        # debug info 
    ...
    for new_col, stop_col in clean_config["stops_cols"].items():
        df_clean[new_col] = raw_data[stop_col].apply(lambda x: get_stops(x, **clean_config["get_stops"]))
