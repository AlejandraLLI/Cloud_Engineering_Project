...
def upload_csv_S3(data: pd.DataFrame, csv_key: str, bucket_name: str) -> None:
    ...
    if bucket_name not in list_buckets:
        logger.error("S3 bucket %s does not exist, the process can't continue. Create the corresponding bucket on your AWS " +
    ...
    try:
        ...
        logger.warning("Error uploading artifact %s to S3. The process will continue without " +
                        "uploading this file. Error: %s", csv_key, err)
    else:
        logger.debug("Artifact %s uploaded to S3 bucket %s.", csv_key, bucket_name)
...
def upload_yaml_S3(data: dict, yaml_key: str, bucket_name: str) -> None:
    """
    Upload a dictionary as a YAML file to the specified S3 bucket.

    Args:
        data: A dictionary to be uploaded.
        yaml_key: The S3 object key (including file name) under which the data should be stored.
        bucket_name: The name of the S3 bucket to upload the data to.
    """
    # Create an S3 client using the default credentials chain
    try:
    ...
    # Write dictionary to yaml string 
    yaml_buffer = StringIO()
    yaml.dump(data, yaml_buffer, default_flow_style=False)

    # Upload files
    try:
        # Upload file to specified S3 bucket
        s3_client.put_object(Bucket=bucket_name, Key = yaml_key, Body = yaml_buffer.getvalue())
    except botocore.exceptions.BotoCoreError as err:
        logger.warning("Error uploading artifact %s to S3. The process will continue without " +
                        "uploading this file. Error: %s", yaml_key, err)
    else:
        logger.debug("Artifact %s uploaded to S3 bucket %s.", yaml_key, bucket_name)

def upload_pkl_S3(data: object, pkl_key: str, bucket_name: str) -> None:
    """
    Upload a python object as a pickle file to the specified S3 bucket.

    Args:
        data: A python object to be uploaded.
        pkl_key: The S3 object key (including file name) under which the data should be stored.
        bucket_name: The name of the S3 bucket to upload the data to.
    """
    # Create an S3 client using the default credentials chain
    try:
    ...
    pkl_buffer = pickle.dumps(data)

    # Upload files
    try:
        # Upload file to specified S3 bucket
        s3_client.put_object(Bucket=bucket_name, Key = pkl_key, Body = pkl_buffer)
    except botocore.exceptions.BotoCoreError as err:
        logger.warning("Error uploading artifact %s to S3. The process will continue without " +
                        "uploading this file. Error: %s", pkl_key, err)
    else:
        logger.debug("Artifact %s uploaded to S3 bucket %s.", pkl_key, bucket_name)